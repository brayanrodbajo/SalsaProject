...
public class FeatureProcessor {
	...
	//added by Brayan
	//Metadata that are to be extracted from the mp3 file
	...
	public FeatureProcessor(int window_size, double window_overlap,
		...
		metadata[0] = "title";
		metadata[1] = "xmpDM:artist";
		metadata[2] = "xmpDM:composer";
...
//			aggContainer.add(aggList);
	...
	public void extractMP3Tags(String wav_path) throws Exception{
		...
		InputStream input = new FileInputStream(new File(mp3_path));
        ContentHandler handler = new DefaultHandler();
        Metadata meda = new Metadata();
        Parser parser = new Mp3Parser();
        ParseContext parseCtx = new ParseContext();
        parser.parse(input, handler, meda, parseCtx);
        input.close();

        // List all metadata
        String[] metadataNames = meda.names();

        values_writer.writeBytes(",");
        String title = meda.get("title");
        if(title!=null)	values_writer.writeBytes(title);
        else values_writer.writeBytes("?");
        values_writer.writeBytes(",");
        String artist = meda.get("xmpDM:artist");
        if(artist!=null) values_writer.writeBytes(artist);
        else values_writer.writeBytes("?");
        values_writer.writeBytes(",");
        String composer = meda.get("xmpDM:composer");
        if(composer!=null) values_writer.writeBytes(composer);
        else values_writer.writeBytes("?");
        values_writer.writeBytes(",");
        String genre = meda.get("xmpDM:genre");
        if(genre!= null) values_writer.writeBytes(genre);
        else values_writer.writeBytes("?");
        values_writer.writeBytes(",");
        String album =meda.get("xmpDM:album");
        if(album!=null) values_writer.writeBytes(album);
        else values_writer.writeBytes("?");
