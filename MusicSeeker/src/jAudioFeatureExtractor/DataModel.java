...
public class DataModel {
	...
	public OutputStream featureKey;
	public OutputStream featureValue;
	/**
	 ...
	 */
	public DataModel(String featureXMLLocation, ModelListener ml, String featureKeyLocation, String featureValueLocation) {
		try{
			featureKey = new FileOutputStream(featureKeyLocation);

			featureValue = new FileOutputStream(featureValueLocation);
			}catch(Exception e){
				e.printStackTrace();
		}
		ml_ = ml;
	...
	public void extract(int windowSize, double windowOverlap,
		...
		// exception
		// if there are none
		Vector <RecordingInfo> recordings = info;
		if (recordings == null)
			throw new Exception(
					"No recordings available to extract features from.");

		if (updater != null) {
			updater.setNumberOfFiles(recordings.size());
		}
...
//		AudioSamples recording_content;
		for (int i = 0; i < recordings.size(); i++) {
			File load_file = new File(recordings.get(i).file_path);
			if (updater != null) {
